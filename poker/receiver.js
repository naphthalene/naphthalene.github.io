...
memoize = function(fn) {
  return function() {
    ...
    if (indexOf.call(fn.memoize, hash) >= 0) {
...
MainState = React.createClass({
  ...
  computeWinner: function() {
    ...
    val = memoize(function(c) {
      return CARDS.indexOf(c.slice(0, -1));
    });
    suit = memoize(function(c) {
      return c.slice(-1)[0];
    });
    evalRank = function(bestPlayer, player, i, a) {
      ...
      combProcess = function(bestHand, ce, ci, ca) {
        var FH, checkStraight, cmp, counts, error, flush, hrank, onePair, quad, quadOrFH, ref, royalFlush, straight, strtVal, trips, tripsOrTwoPair, twoPair, twoPairFinder;
        try {
  ...
  awardPotTo: function(winners) {
    var dividend, p, player_index, players;
    players = this.state.players;
    console.log(winners);
  ...
  playerAction: function(sender, action, updateFunc) {
    var p, player_index, players, success;
    player_index = this.state.players.map(function(e) {
      return e.id;
    }).indexOf(sender);
    players = this.state.players;
  ...
  raisePlayer: function(sender, data) {
    var t;
    t = this;
    return this.playerAction(sender, "raise", function(p, player_index) {
      var e, error, withdraw;
      try {
  ...
  callPlayer: function(sender) {
    var t;
    t = this;
    return this.playerAction(sender, "call", function(p, player_index) {
      var withdraw;
  ...
  checkPlayer: function(sender) {
    var t;
    t = this;
    return this.playerAction(sender, "check", function(p, player_index) {
  ...
  dealHand: function(dealer) {
    var bid, bigBlind, e, error, error1, firstTurn, i, l, len1, p, player, players, ref, smallBlind;
    players = [];
    i = 0;
...
table = {
  ...
  handleMessage: function(sender, m) {
    var e, error, isReconnecting, player_info, reconnect, reduceFun;
